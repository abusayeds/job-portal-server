...
const createUserDB = async (payload: IUser) => {
  const isUserRegistered: IUser | null = await UserModel.findOne({
    email: payload.email,
    isVerify: true,
  });
  if (isUserRegistered) {
  ...
  const token = jwt.sign({ email, otp: otp }, JWT_SECRET_KEY as string, {
    expiresIn: "7d",
  });
  return {
    token: token,
...
const verifyOtpDB = async (email: string) => {
  ...
  const userObj = (verifiedUser as any).toObject
    ? (verifiedUser as any).toObject()
    : verifiedUser;
...
const loginDB = async (email: string, password: string) => {
  ...
  if (!user.isActive && !isAllowedEmployer) {
    throw new AppError(
      httpStatus.NOT_FOUND,
      "Your account is inactive by admin.",
    );
  }
...
const resendOtpDB = async (email: string) => {
  ...
  const token = jwt.sign(
    { email, forgot: "forgot", otp: newOTP },
    JWT_SECRET_KEY as string,
    { expiresIn: "7d" },
  );
  return token;
...
const myProfileDB = async (userId: string) => {
  const user: IUser | null = await UserModel.findOne({
    ...
    .populate({
      path: "employmentId",
      select: "references professional_skills employments  -_id ",
    })
    .populate({
      path: "purchasePlan",
      select: "-createdAt -updatedAt -__v -isVisible",
  ...
  if (user.role === "candidate") {
    const { candidateInfo, employmentId, ...userWithoutCandidateInfo } =
      user.toObject();
    const flattenedUser = {
      ...userWithoutCandidateInfo,
      ...candidateInfo,
      _id: user._id,
      ...employmentId,
    };
...
const allUserDB = async (query: Record<string, unknown>, role: string) => {
  const userQuery = new queryBuilder(
    UserModel.find({ role: role, isDeleted: false })
...
const IdentityVerificationDB = async (
  id: string,
  payload: IUser,
  step: string,
) => {
...
const approveEmployerDB = async (query: Record<string, unknown>) => {
  ...
  const userQuery = new queryBuilder(
    UserModel.find(filters).select("-password -isVerify"),
    query,
  )
    .fields()
    .filter()
...
const accessEmployeDB = async (payload: IUser, employerId: string) => {
  const employer =
    await UserModel.findById(employerId).populate("purchasePlan");
  if (employer?.purchasePlan?.planName !== "unlimited_plan") {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      ` Access cannot be granted to anyone under the ${employer?.purchasePlan?.planName}`,
    );
  }
  ...
  if (maxEmploye.length >= 3) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      "Cannot assign more than 3 employees to this plan.",
    );
  }
